...
const sortedCustomers = computed(() => {
  return [...customers.value].sort((a, b) => {
    ...
    return (getMs(b.updatedAt) || getMs(b.createdAt)) - (getMs(a.updatedAt) || getMs(a.createdAt))
  })
})
...
const emptyAdd = () => ({ name: '', email: '', phone: '', ic: '' })
...
const handleAddDocSelect = (e) => {
    ...
    if (!file) return
    addDocFile.value = file
    addDocPreview.value = file.type.startsWith('image/') ? URL.createObjectURL(file) : ''
}

const openAddModal = () => {
    addForm.value = emptyAdd()
    addDocFile.value = null
    addDocPreview.value = ''
    isAddModalOpen.value = true
}

const openEditModal = (customer) => { selectedCustomerId.value = customer.id }
...
const handleAddCustomer = async () => {
    ...
    try {
        ...
        if (addDocFile.value && newId) {
            const bizId = authStore.user?.businessId
            const file = addDocFile.value
            const path = `business/${bizId}/customers/${newId}/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`
            const ref = storageRef(storage, path)
            await uploadBytes(ref, file, { contentType: file.type })
            const url = await getDownloadURL(ref)
            await customersStore.addDocument(newId, {
                name: file.name, url, path, type: file.type,
...
<template>
  <section>
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="border-b border-gray-100 dark:border-gray-700">
            ...
            <th class="p-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">IC Number</th>
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
          <tr v-for="customer in tableCustomers" :key="customer.id"
            ...
            <td class="p-4">
              <span class="text-sm font-mono text-gray-600 dark:text-gray-400">{{ customer.ic || '—' }}</span>
            </td>
            <td class="p-4">
            ...
            <td class="p-4 text-right">
              <div class="flex items-center justify-end gap-2">
                ...
                <a v-if="customer.phone" :href="`https://wa.me/${customer.phone.replace(/[^0-9]/g, '')}`" target="_blank" @click.stop class="p-2 text-gray-400 hover:text-green-600 dark:hover:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg transition-colors" title="WhatsApp">
                ...
                <button @click.stop="openEditModal(customer)" class="p-2 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg transition-colors" title="Edit">
          ...
          <tr v-if="tableCustomers.length === 0">
            <td colspan="6" class="p-8 text-center text-gray-500 dark:text-gray-400 text-sm">No customers found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <Teleport to="body">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input v-model="addForm.name" type="text" placeholder="e.g. Sarah Connor" class="w-full pl-10 p-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:bg-white dark:focus:bg-gray-600 focus:ring-2 focus:ring-[#4DB6AC] outline-none transition-all text-gray-800 dark:text-white">
                    </div>
                </div>
                <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="addForm.ic" type="text" placeholder="e.g. 900101-14-1234" class="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:bg-white dark:focus:bg-gray-600 focus:ring-2 focus:ring-[#4DB6AC] outline-none transition-all text-gray-800 dark:text-white font-mono">
                    </div>
                </div>
                <!-- Optional document upload -->
                <div>
                    <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Attach Document (Optional)</label>
                    <div @click="addDocInput.click()"
                        class="flex items-center gap-3 border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-lg p-3 cursor-pointer hover:border-teal-400 dark:hover:border-teal-500 hover:bg-teal-50 dark:hover:bg-teal-900/10 transition-colors">
                        <template v-if="!addDocFile">
                            <svg class="w-5 h-5 text-gray-300 dark:text-gray-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            <span class="text-xs text-gray-400 dark:text-gray-500">Click to upload IC, passport or agreement</span>
                        </template>
                        <template v-else>
                            <img v-if="addDocPreview" :src="addDocPreview" class="w-10 h-10 object-cover rounded shrink-0" alt="" />
                            <div v-else class="w-10 h-10 bg-gray-100 dark:bg-gray-700 rounded flex items-center justify-center shrink-0">
                                <span class="text-[10px] font-black text-gray-500">{{ addDocFile.name.split('.').pop().toUpperCase() }}</span>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-bold text-gray-700 dark:text-gray-300 truncate">{{ addDocFile.name }}</p>
                                <p class="text-[10px] text-gray-400">Will upload after profile is created</p>
                            </div>
                            <button type="button" @click.stop="addDocFile = null; addDocPreview = ''" class="text-gray-300 hover:text-red-500 text-lg leading-none">&times;</button>
                        </template>
                    </div>
                    <input ref="addDocInput" type="file" class="hidden" accept="image/*,.pdf,.doc,.docx" @change="handleAddDocSelect" />
                </div>
      ...
      <CustomerDetailModal :customerId="selectedCustomerId" @close="selectedCustomerId = null" />
    </Teleport>
        ...
        <div class="relative bg-gray-50 dark:bg-gray-900 w-full max-w-5xl h-[90vh] rounded-xl shadow-2xl flex flex-col animate-fade-in-up overflow-hidden border border-gray-200 dark:border-gray-700">
          ...
          <div class="px-6 py-3 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shrink-0">
            <div class="relative">
              ...
              <input v-model="searchQuery" type="text" placeholder="Search by name, phone, email or IC…" class="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#4DB6AC] transition-colors" />
            </div>
          </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700">
              <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-400 sticky top-0 z-10">
                  <tr class="border-b border-gray-100 dark:border-gray-700">
                    <th class="p-4 text-xs font-bold uppercase tracking-wider">Customer</th>
                    <th class="p-4 text-xs font-bold uppercase tracking-wider">Contact</th>
                    <th class="p-4 text-xs font-bold uppercase tracking-wider">IC Number</th>
                    <th class="p-4 text-xs font-bold uppercase tracking-wider">Status</th>
                    <th class="p-4 text-xs font-bold uppercase tracking-wider">Joined</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700 align-top">
                  <tr v-if="filteredCustomers.length === 0">
                    <td colspan="5" class="p-8 text-center text-gray-400 dark:text-gray-500">No customers found.</td>
                  </tr>
                  <tr v-for="customer in filteredCustomers" :key="customer.id"
                    @click="openEditModal(customer); isViewAllModalOpen = false"
                    class="hover:bg-gray-50/80 dark:hover:bg-gray-700/50 transition-colors cursor-pointer">
                    <td class="p-4">
                      <div class="flex items-center gap-3">
                        <div :class="[customer.color, 'w-9 h-9 rounded-full flex items-center justify-center font-bold text-xs shadow-sm']">{{ customer.initials }}</div>
                        <div>
                          <div class="font-bold text-gray-800 dark:text-white text-sm">{{ customer.name }}</div>
                          <div class="text-xs text-gray-500 dark:text-gray-400">{{ customer.email || '—' }}</div>
                        </div>
                    ...
                    <td class="p-4 text-sm text-gray-600 dark:text-gray-400">{{ customer.phone || '—' }}</td>
                    <td class="p-4 text-sm font-mono text-gray-600 dark:text-gray-400">{{ customer.ic || '—' }}</td>
                    <td class="p-4">
                      <span :class="customer.status === 'Active' ? 'text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30' : 'text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30'"
                            class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide">{{ customer.status }}</span>
                    </td>
                    <td class="p-4 text-sm text-gray-600 dark:text-gray-400">{{ customer.joinedDate }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
